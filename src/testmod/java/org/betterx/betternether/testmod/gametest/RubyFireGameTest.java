...
public class RubyFireGameTest {
    ...
    /**
     * The same retaliation, but with a player swinging instead of a mob. Ruby Fire is supposed to hit
     * whoever attacks the wearer, and a player is an attacker like any other.
     * <p>
     * The knockback half of that is not asserted here: it needs ApplyEntityImpulse, which this version
     * of Minecraft does not have, so this branch only burns and hurts its attackers.
     */
    @GameTest(maxTicks = 400)
    public void rubyFireBurnsAPlayerAttacker(GameTestHelper helper) {
        final ServerPlayer attacker = MockPlayers.survival(helper, ATTACKER_POS);
        final float fullHealth = attacker.getHealth();

        boolean burned = false;

        for (int attempt = 0; attempt < ATTEMPTS && !burned; attempt++) {
